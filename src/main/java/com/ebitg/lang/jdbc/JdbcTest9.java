...
class T9_1 implements Runnable {
	@Override
	public void run() {
	...
	private ArrayList<Integer> queryAll(Connection conn) throws ClassNotFoundException, SQLException {
		String sql = "select * from T1 where updateCount=0  limit 50";
	...
	private int update(Connection conn, int id) {
		try {
			String updateSql = "update T1 set updateTime=current_time(),updateCount=updateCount+1 where id=?";
