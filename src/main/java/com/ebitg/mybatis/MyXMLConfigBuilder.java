...
public class MyXMLConfigBuilder extends BaseBuilder {
	...
	protected static final String DEFAULT_SQLXML_SUFFIX = "_sql.xml";
	...
	public MyXMLConfigBuilder(InputStream inputStream, String environment) {
		...
		if (parent != null) {
			for (XNode child : parent.getChildren()) {
				if ("package".equals(child.getName())) {
					...
					String xmlBasePath = child.getStringAttribute("xmlBasePath");
					...
					} else if (resource == null && url == null && mapperClass == null && xmlBasePath != null) {
						File sqlXmlDir = Resources.getResourceAsFile(xmlBasePath);
						String tempSqlMapperFileSuffix = child.getStringAttribute("xmlBasePath");
						if (StringUtils.isBlank(tempSqlMapperFileSuffix)) {
							tempSqlMapperFileSuffix = DEFAULT_SQLXML_SUFFIX;
						}
						String sqlMapperFileSuffix = tempSqlMapperFileSuffix;
						File[] sqlXmlFiles = sqlXmlDir.listFiles(new FilenameFilter() {
							@Override
							public boolean accept(File dir, String name) {
								return StringUtils.endsWithIgnoreCase(name, sqlMapperFileSuffix);
							}
						});

						for (int i = 0; i < sqlXmlFiles.length; i++) {
							XMLMapperBuilder mapperParser = new XMLMapperBuilder(new FileInputStream(sqlXmlFiles[i]),
									configuration, sqlXmlFiles[i].getName(), configuration.getSqlFragments());
							mapperParser.parse();
						}
					} else {
						throw new BuilderException(
